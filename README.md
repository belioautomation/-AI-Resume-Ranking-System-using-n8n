# 📄 AI Resume Ranking System using n8n

An AI-powered recruitment automation workflow built with **n8n** and **Google Gemini AI**. This project automatically monitors a Google Drive folder for newly uploaded resumes, extracts the document text, evaluates each candidate using AI, assigns a score and ranking, stores the results in Google Sheets, and sends Telegram notifications for qualified applicants.

Designed as part of my **30-Day n8n Automation Portfolio**, this workflow demonstrates how AI can streamline resume screening and candidate evaluation.

---

# 📌 Features

- 📂 Automatically monitors Google Drive for new resumes
- 📥 Downloads uploaded PDF resumes
- 📄 Extracts resume text from PDF files
- 🤖 Uses Google Gemini AI to evaluate candidates
- ⭐ Assigns an AI-generated candidate score
- 🏆 Ranks applicants based on qualifications
- 📊 Logs applicant evaluations to Google Sheets
- 📲 Sends Telegram notifications for qualified candidates
- 🧠 Uses structured JSON output for reliable workflow automation

---

# 🛠 Technologies Used

- n8n
- Google Drive Trigger
- Google Drive
- Extract From File
- Google Gemini AI
- Code Node (JSON Parser)
- IF Node
- Google Sheets API
- Telegram Bot API

---

# 📂 Workflow

```text
Google Drive Trigger
      │
      ▼
Download File
      │
      ▼
Extract From File
      │
      ▼
AI Agent (Google Gemini)
      │
      ▼
Code (Parse AI JSON)
      │
      ▼
IF (Score ≥ 80?)
 ┌──────────────┐
 ▼              ▼
Qualified   Needs Review
 │              │
 ▼              ▼
Google Sheets Google Sheets
 │
 ▼
Telegram
```

---

# ⚙ Workflow Explanation

## 1. Google Drive Trigger

Monitors a Google Drive folder for newly uploaded PDF resumes.

Configuration:

- Event: File Created
- Folder: Job Applications

---

## 2. Download File

Downloads the uploaded PDF from Google Drive for processing.

---

## 3. Extract From File

Extracts all text content from the uploaded PDF resume.

---

## 4. AI Agent (Google Gemini)

Google Gemini analyzes the resume and evaluates the candidate based on:

- Education
- Technical Skills
- Work Experience
- Certifications
- Overall Candidate Quality

Expected AI Output:

```json
{
  "name": "John Doe",
  "email": "john@example.com",
  "phone": "+63 912 345 6789",
  "education": "Bachelor of Science in Information Technology",
  "experience": "2 years IT Support",
  "skills": [
    "Networking",
    "Windows",
    "Python"
  ],
  "certifications": [
    "CompTIA A+"
  ],
  "score": 88,
  "rank": "Qualified Candidate",
  "recommendation": "Interview",
  "summary": "Strong IT support candidate with relevant technical skills."
}
```

---

## 5. Code Node

Parses the JSON response generated by Google Gemini into structured data that n8n can process.

---

## 6. IF Node

Checks:

```text
Score ≥ 80
```

If **True**

- Save to **Qualified Candidates**
- Send Telegram notification

If **False**

- Save to **Needs Review**

---

## 7. Google Sheets

Stores all evaluated candidates.

### Qualified Candidates

Stores applicants who achieved a score of **80 or higher**.

### Needs Review

Stores applicants who scored below **80** for future evaluation.

---

## 8. Telegram

Sends an instant notification whenever a qualified candidate is detected.

Example:

```text
📄 Qualified Candidate Detected

👤 Name:
John Doe

⭐ Score:
88 / 100

🏆 Rank:
Qualified Candidate

📌 Recommendation:
Interview

📝 Summary:
Strong IT support candidate with relevant technical skills.

🤖 Evaluated automatically using Google Gemini.
```

---

# 📊 Google Sheets Structure

| Timestamp | Name | Email | Phone | Education | Score | Rank | Recommendation | Summary |
|----------|------|-------|-------|-----------|------:|------|----------------|---------|

---

# 📁 Repository Structure

```text
AI-Resume-Ranking-System/
│
├── README.md
├── workflow.json
│
├── screenshots/
│   ├── workflow.png
│   ├── google-drive-trigger.png
│   ├── extract-from-file.png
│   ├── ai-agent-output.png
│   ├── code-node-output.png
│   ├── if-node.png
│   ├── google-sheets.png
│   ├── telegram-notification.png
│   └── workflow-execution.png
│
└── assets/
    └── sample-output.json
```

---

# 📷 Screenshots

Include the following screenshots:

- Complete Workflow
- Google Drive Trigger
- Extract From File Output
- AI Agent Output
- Code Node Output
- IF Node Execution
- Google Sheets Results
- Telegram Notification

---

# 🎯 Learning Objectives

This project demonstrates:

- Google Drive Automation
- PDF Text Extraction
- AI Resume Evaluation
- Prompt Engineering
- Structured JSON Parsing
- Conditional Workflow Logic
- Google Sheets Automation
- Telegram Automation
- AI-Powered Candidate Screening

---

# 🚀 Possible Improvements

- Support DOCX resumes
- Extract portfolio and LinkedIn URLs
- AI interview question generation
- Candidate comparison dashboard
- Automatic email invitations
- Resume duplicate detection
- Multi-language resume support
- HR analytics dashboard
- Integration with Notion or Airtable

---

# 📄 License

This project is licensed under the MIT License.

---

# 🙌 Acknowledgements

- n8n
- Google Gemini AI
- Google Drive API
- Google Sheets API
- Telegram Bot API
